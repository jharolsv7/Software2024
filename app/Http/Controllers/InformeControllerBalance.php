...
class InformeControllerBalance extends Controller
{
    public function generarPDF(Request $request)
    {
       ...
       $options->set('isPhpEnabled', true); // Habilitar el soporte PHP
       $options->set('isRemoteEnabled', true); // Permitir la carga de imágenes remotas
       $options->set('DOMPDF_ENABLE_REMOTE', true); // Habilitar la carga de imágenes remotas
       $options->set('isHtml5ParserEnabled', true); // Habilitar el analizador HTML5
       $options->set('chroot', base_path()); // Establecer la ruta base
       $options->set('defaultPaperSize', 'A4'); // Establecer el tamaño de papel predeterminado
       ...
       $html = '<html><head><style>
           ...
           .container { margin: 0 auto; max-width: 800px; } /* Añadido */
       ...
       $html .= '<h1>Informe de Balance - Software League</h1>';
       $html .= '<div class="fecha">Fecha del informe: ' . $fechaActual . '</div>';
       $html .= '<table>';
       $html .= '<tr><th>Tipo</th><th>Total</th></tr>';
       $html .= '<tr><td>Ingresos</td><td>$' . number_format($totalIngresos, 2) . '</td></tr>';
       $html .= '<tr><td>Egresos</td><td>$' . number_format($totalEgresos, 2) . '</td></tr>';
       $html .= '<tr><td class="total">Balance</td><td class="total">$' . number_format($balance, 2) . '</td></tr>';
       $html .= '</table>';
