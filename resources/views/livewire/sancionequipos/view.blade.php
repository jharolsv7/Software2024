		...
		<div class="col-md-12">
			<div class="card">
				<div class="card-header">
					<div style="display: flex; justify-content: space-between; align-items: center;">
						<div class="float-left">
							...
							Tabla Sancion de Equipo </h4>
						</div>
						@if (session()->has('message'))
						<div wire:poll.4s class="btn btn-sm btn-success" style="margin-top:0px; margin-bottom:0px;"> {{ session('message') }} </div>
						@endif
						<div>
							<input wire:model='keyWord' type="text" class="form-control" name="search" id="search" placeholder="Buscar Sancion Equipos">
						</div>
						<div class="btn btn-sm btn-info" data-bs-toggle="modal" data-bs-target="#createDataModal">
						<i class="fa fa-plus"></i>  Agregar una Sancion al Equipo
						</div>
					</div>
				</div>
				
				<div class="card-body">
						@include('livewire.sancionequipos.modals')
				<div class="table-responsive">
					...
					<div class="float-end">{{ $sancionequipos->links() }}</div>
					</div>
				</div>
			</div>
		</div>
